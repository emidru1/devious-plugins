...
public enum NhBirdhousesSpace {
    ...
    public static TileObject getNearest(final NhBirdhousesObjectState state) {
        TileObject birdHouse = null;

        for (NhBirdhousesSpace space : NhBirdhousesSpace.values()) {
            if (NhBirdhousesObjectState.fromVarpValue(Vars.getVarp(space.getVarp().getId())) == state) {
